...
1. **datacompleteness**: Whether a single player's statistics in a single eSport match in the original dataset is **filled** before we did data cleaning. 
...
2. Each team has **5 players** and a match contains **2 teams** and so 10 players. After examining the file, we find that **every 12 rows** represent a single match, **every 11th row and 12th row** represents the 2 teams' overall statistics, and therefore the **10 rows before them** represents these 2 teams' 10 individual players' statistics of this match. As our goal is to examine individual performances of top laners and mid laners, we drop all rows representing the statistics of whole teams.

3. The 'datacompleteness' column contains 3 kinds of values: 'complete', 'ignore', and 'partial'.
...
In the below **univariate histogram**, the x-axis shows standardized KDA in standardized units, ranging from -1 to 8, with a bin size of 0.5 standardized units for better vidualization. The y-axis shows the amount of players with their specific standardized KDA.
The graph is skewed to the right, inferring that most players' standardized KDA is in the range of -1 ~ 0 standardized unit, which further means that a lot of players' KDA is relatively lower compared to the average value within the dataset. At the same time few players have very high standardized KDA such as 7 or 8, showing that there is a huge difference between players' standardized KDA.
...
The table above demonstrates the **mean statistics** for each position in League of Legends and can be used for comparison among different positions. Also, it provides a more specific numerical illustration of the statistics of the five positions. For example, mid and top are the two positions with the smallest difference in mean standardized carry score.
...
We don't believe that there is column in our dataset that is NMAR because we believe that the missingness of column **teamname** may depend on other columns in this dataset. 

We would like to obtain additional information about **possible correlation** between **datacompleteness** column and missingness in **teamname** column, as well as **position** column and missingness in **teamname** column, in order to see the type of missingness of the **teamname** column. 

In League of Legends, many variables can influence the missingness of columns in the dataset including the team roster changes, the type of professional competitions like regional playoffs versus world championship, incomplete games with unexpected technical difficulties, etc.
...
* After obtaining our p-value for this permutation testing: 0.004, which is smaller than the significance level, we **reject the null hypothesis**.
...
* After obtaining the our p-value for this permutation testing: 1.0, which is bigger than the significance level, we **fail to reject the null hypothesis**. 

* The result of this permutation testing is that the missingness of **teamname** column does not depend on **position** column, which means the missingness of **teamname** is MCAR when it is analyzed with and only with **position** column.
